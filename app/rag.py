...
# ---------------- INIT ----------------
def init_llm():
    """Initialize LLM and lightweight embedding model"""
    global llm, embeddings
    ...
    # ✅ Lightweight embeddings (Render safe)
    embeddings = HuggingFaceEmbeddings(
        model_name="sentence-transformers/paraphrase-MiniLM-L3-v2",
        model_kwargs={"device": "cpu"},
...
def load_faiss_index() -> bool:
    ...
    retriever = db.as_retriever(
        search_type="mmr",
        search_kwargs={"k": 3}   # ✅ reduced memory usage
    )
...
def is_domain_question_semantic(query: str, threshold: float = 0.30) -> bool:
    ...
    similarity = cosine_similarity([query_vec], [domain_vec])[0][0]
...
STRICT RULES:
- Answer ONLY using the Retrieved Information.
- Answer ONLY questions related to coal mining laws.
- DO NOT hallucinate.
